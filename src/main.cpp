...
int main()
{
    ...
    // load pbr equirectangular map
    stbi_set_flip_vertically_on_load(true);
    int width, height, nComponents;
    float* data = stbi_loadf("../../../textures/newport_loft.hdr", &width, &height, &nComponents, 0);
    ...
    // allocate cubemap color textures
    ...
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR); // enable prefilter mipmap sampling(against visible dots artefacts)
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    // capture projection and positions
    ...
    glViewport(0, 0, 512, 512); // confire viewport before capturing
    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    for (unsigned int i = 0; i < 6; ++i)
    {
    ...
    // then let opeongl generate mipmaps from first mip face(against visible dots artefacts)
    glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap);
    ...
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // generate mipmaps for cubemap texture to opengl can automatically allocate the required memory
    glGenerateMipmap(GL_TEXTURE_CUBE_MAP);

    // ------ pbr: run quasi monte-carlo sumulation on the environment lighting to create a prefilter (cube)map
    prefilterShader.use();
    ...
    {
        // resize framebuffer according to mipmap size
    ...
    // set viewport back to normal after rendering cubemap textures
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        /*equirectangularShader.use();
        equirectangularShader.setMat4("view", camera.GetViewMatrix());
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, hdrTexture);
        renderCube();*/
        ...
        {
            // we clamp the roughness to 0.025 - 1.0  as perfectly smooth surface(roughness 0.0) tend to look a bit off
            // on direct lighting
        ...
        // this looks a bit off as we use the same shader, but itll make their positions obvious and keeps the code small
        ...
        //draw environment/background/skybox
        backgroundShader.use();
